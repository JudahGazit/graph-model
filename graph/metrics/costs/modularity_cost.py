...
class ModularityCost(ICost):
    def categorise_node(self, node, number_of_modules):
        n = int(np.sqrt(self.graph_dataset.number_of_nodes))
        module_size = n // number_of_modules
        position = np.asarray(self.graph_dataset.positions(node)) // module_size
        return int(''.join(position.astype('str').tolist()))

    def cost(self):
        number_of_modules = 2
        categorise_nodes = np.array([self.categorise_node(node, number_of_modules) for node in range(self.graph_dataset.number_of_nodes)])
        edge_sources, edge_targets = np.nonzero(self.graph_dataset.adjacency)
